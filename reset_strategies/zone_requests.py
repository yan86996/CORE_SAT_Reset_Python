...
class Requests:
    ...
    """
    def __init__(self, verbose=False, ignore_zones=[], important_clg=[], important_htg=[], folder_dir = None, zone_names=None, zone_dev_map = None, room_temp=None, clg_setpoint=None, htg_setpoint=None,     
        ...
        self.important_clg = important_clg
        self.important_htg = important_htg
        self.zone_dev_map = zone_dev_map
        self.zd_clg = {} # dict object representing all of the necessary zone data
        self.zd_htg = {} # dict object representing all of the necessary zone data
...
class Clg_Request(Requests):
    ...
    def update(self):
        ...
        for z in self.zd_clg:
            if 'clg_requests' in self.zd_clg[z]:
                del self.zd[z]['cooling_loop']
            if 'cooling_loop' in self.zd_clg[z]:
                del self.zd[z]['cooling_loop']
            if 'room_temp' in self.zd_clg[z]:
                del self.zd_clg[z]['room_temp']
            if 'clg_setpoint' in self.zd_clg[z]:
                del self.zd_clg[z]['clg_setpoint']
        
        for zone_name in self.zone_names:
            self.zd_clg[zone_name] = {}
            try:
                # get zone data
                ...
                self.zd_clg[zone_name]['min_flow'] = min_flow
          
                # max airflow
                max_flow = zone_data['Present_Value'][np.char.find(zone_data['Object_Name'], self.flow_max) >= 0][0]
                self.zd_clg[zone_name]['max_flow'] = max_flow  
          
                # room temp
                self.zd_clg[zone_name]['room_temp'] = zone_data['Present_Value'][np.char.find(zone_data['Object_Name'], self.room_temp) >= 0][0] 
          
                # cooling setpoint
                self.zd_clg[zone_name]['clg_setpoint'] = zone_data['Present_Value'][np.char.find(zone_data['Object_Name'], self.clg_setpoint) >= 0][0] 
            ...
            try:    
                ...
                self.zd_clg[zone_name]['flow'] = airflow
                
                # cooling loop
                self.zd_clg[zone_name]['cooling_loop'] = (self.zd_clg[zone_name]['flow'] - min_flow)/(max_flow - min_flow)
        ...
        for z in sorted(self.zd_clg):
            if 'cooling_loop' in self.zd_clg[z]:
                if self.zd_clg[z]['room_temp'] > self.zd_clg[z]['clg_setpoint']:
                    self.c_clg += 1   
                    
                if self.zd_clg[z]['cooling_loop'] < .95:
                    self.zd_clg[z]['clg_requests'] = 0
                    
                if self.zd_clg[z]['cooling_loop'] >= .95:
                    self.zd_clg[z]['clg_requests'] = 1               
                    if self.zd_clg[z]['room_temp'] >= self.zd_clg[z]['clg_setpoint'] + 3.0:
                        self.zd_clg[z]['clg_requests'] = 2    
                    if self.zd_clg[z]['room_temp'] >= self.zd_clg[z]['clg_setpoint'] + 5.0:
                        self.zd_clg[z]['clg_requests'] = 3
                  
                    if self.low_temp_cutoff:
                        if self.zd_clg[z]['clg_setpoint'] <= self.low_temp_cutoff:
    ...
    def calc_Clg_TotalRequests(self):
        ...
        for z in self.zd_clg:  
            if 'clg_requests' in self.zd_clg[z]:
                self.rR_clg += self.zd_clg[z]['clg_requests']

                if 'importance_clg' in self.zd_clg[z]:
                    self.R_clg += self.zd_clg[z]['clg_requests'] * float(self.zd_clg[z]['importance_clg'])
                else:
                    self.R_clg += self.zd_clg[z]['clg_requests']               
...
class Htg_Request(Requests):
    ...
    def update(self):
        ...
        self.missingEssential = []
        # clear existing zone data from previous update
        for z in self.zd_htg:
            if 'htg_requests' in self.zd_htg[z]:
                del self.zd_htg[z]['htg_requests']
            if 'room_temp' in self.zd_htg[z]:
                del self.zd_htg[z]['room_temp']
            if 'htg_setpoint' in self.zd_htg[z]:
                del self.zd_htg[z]['htg_setpoint']
                
        for zone_name in self.zone_names:
            self.zd_htg[zone_name] = {}
            try: 
                # get zone data
                div_ID = self.zone_dev_map[zone_name]
                zone_csv = os.path.join(self.folder_dir, f'AV_{div_ID}.csv')
                ...
                # min airflow
                min_flow = zone_data['Present_Value'][np.char.find(zone_data['Object_Name'], self.flow_min) >= 0][0]
                self.zd_htg[zone_name]['min_flow'] = min_flow
          
                # max airflow
                max_flow = zone_data['Present_Value'][np.char.find(zone_data['Object_Name'], self.flow_max) >= 0][0]
                self.zd_htg[zone_name]['max_flow'] = max_flow  
          
                # room temp
                self.zd_htg[zone_name]['room_temp'] = zone_data['Present_Value'][np.char.find(zone_data['Object_Name'], self.room_temp) >= 0][0]
                
                # heating setpoint
                self.zd_htg[zone_name]['htg_setpoint'] = zone_data['Present_Value'][np.char.find(zone_data['Object_Name'], self.htg_setpoint) >= 0][0] 
            except Exception as e:
                print(e)
                print(f'missing data in AV_{div_ID}.csv')
            ...
            try: 
                ...
                # airflow
                airflow = zone_data_AI['Present_Value'][np.char.find(zone_data_AI['Object_Name'], self.flow) >= 0][0]
                self.zd_htg[zone_name]['flow'] = airflow
        ...
        self.c_htg = 0
        # calculate cooling requests
        for z in sorted(self.zd_htg):
            if self.zd_htg[z]['room_temp'] < self.zd_htg[z]['htg_setpoint']:
                self.zd_htg[z]['htg_requests'] = 1    
                self.c_htg += 1   
            if self.zd_htg[z]['room_temp'] <= self.zd_htg[z]['htg_setpoint'] - 3.0:
                self.zd_htg[z]['htg_requests'] = 2    
            if self.zd_htg[z]['room_temp'] <= self.zd_htg[z]['htg_setpoint'] - 5.0:
                self.zd_htg[z]['htg_requests'] = 3
            
            if self.low_temp_cutoff:
                if self.zd_htg[z]['htg_setpoint'] >= self.high_temp_cutoff:
                    #ignore requests from zone with a setpoint below a self.low_temp_cutoff
                    self.zd_htg[z]['htg_requests'] = 0
                else:
                    self.missingPartial.append(z)
    ...
    def calc_Htg_TotalRequests(self):
        ...
        for z in self.zd_htg:  
            if 'htg_requests' in self.zd_htg[z]:
                self.rR_htg += self.zd_htg[z]['htg_requests']
                                
                if 'importance_clg' in self.zd_htg[z]:
                    self.R_htg += self.zd_htg[z]['htg_requests'] * float(self.zd_htg[z]['importance_clg'])
                else:
                    self.R_htg += self.zd_htg[z]['htg_requests']
    ...
    def displayDetails(self):
        ...
        print('\n================= Details for zones with cooling loop almost at maximum ================ ')
        for z in sorted(self.zd_htg):
            if 'cooling_loop' in self.zd_htg[z]:
                if self.zd_htg[z]['cooling_loop'] >= 95:
                    print(str(z))
