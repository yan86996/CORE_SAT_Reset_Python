...
<<<<<<< HEAD
            ...
            self.ts_header += [vav+' rhv power for lo SAT', vav+' chw power for cur SAT', vav+' chw power for ho SAT', ] # log
...
<<<<<<< HEAD
                    ...
                    diff_rhv_power = self.calc_heat_flow(self, new_zone_afr, cur_sat_sp+diff_sat)
                    self.ts_data += np.concatenate([arr.flatten() for arr in diff_rhv_power]).tolist() # log 
                    self.estimations['rhv_clo_temp_chg_' + vav] = 0
                    self.estimations['rhv_power_delta_' + vav] = self.calc_heat_flow(self, new_zone_afr, -diff_sat)  
...
<<<<<<< HEAD
    ...
    # def log_data_old (self, var_in_csv, data_type, ahu_name, ahu_data_AV, new_ahu_data_AV, estimations):      
    #     if not any(var_in_csv in item for item in ahu_data_AV['Object_Name']):
    #         new_nparry = np.array([('9999999', data_type, '9999999', var_in_csv+' '+ahu_name, 
    #                                 estimations[var_in_csv],'°F')], dtype=new_ahu_data_AV.dtype)                                                         
    #         new_ahu_data_AV = np.append(new_ahu_data_AV, new_nparry)
            
    #     else:
    #         idx_find = [np.char.find(ahu_data_AV['Object_Name'], var_in_csv)>=0][0]
    #         new_ahu_data_AV['Present_Value'][idx_find] = estimations[var_in_csv]
...
<<<<<<< HEAD
            ...
            ###
            new_zone_afr = afr * (clg_sp - cur_sat - 3) / (clg_sp - (cur_sat + diff_sat) - 3)
=======
            ### actual deltaT vs assumed deltaT to be checked later
